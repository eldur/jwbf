...
class WatchList extends BaseQuery<WatchResponse> {
    ...
    private WatchList.Builder builderOf(WatchList watchList) {
        return WatchList.from(bot()) //
                ...
                .onlyTypes(watchList.editType) //
                .onlyUser(watchList.user) //
                .owner(watchList.owner, watchList.token) //
                .showAnonymous(watchList.showAnonymous) //
                .showBots(watchList.showBots) //
                .showMinor(watchList.showMinor) //
                .withDir(watchList.dir) //
                .withEnd(watchList.end) //
                .withStart(watchList.start) //

                //.withProperties(watchList.properties)
                //.withNamespaces(watchList.namespaces)
                //.withLimit(watchList.limit) //
    ...
    protected HttpAction prepareNextRequest() {
        ...
        if (start != null) {
            requestBuilder.param("wlstart", WatchList.formatDate(start));
        }
        if (end != null) {
            requestBuilder.param("wlend", WatchList.formatDate(end));
        }
        if (dir != null) {
