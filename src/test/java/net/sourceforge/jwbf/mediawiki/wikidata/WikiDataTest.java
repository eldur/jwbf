...
public class WikiDataTest {
  ...
  private static class GetClaims extends MWAction implements JsonMapper.ToJsonFunction {
    ...
    public String processAllReturningText(String text) {
      result = text;
      try {
        Files.write(text, new File("/home/loki/git/wikidata-toolkit/response.json"), Charsets.UTF_8);
      } catch (IOException e) {
        e.printStackTrace();
      }
    ...
    public Object toJson(@Nonnull String jsonString, Class<?> clazz) {
      ...
      ItemDocument documents =
          jsonConverter.convertToItemDocument(new JSONObject(jsonString), itemIdString);
      /*
