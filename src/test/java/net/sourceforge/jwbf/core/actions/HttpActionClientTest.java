...
public class HttpActionClientTest {
	...
	public void testMissingUseragent() {
		...
		// THEN
		GAssert.assertEquals(ImmutableList
				.<String> of("[WARN] a User-Agent must be set in your client"),
				logLinesSupplier.get());
	}
	...
	public void testPerformAction() {
		...
		try {
			...
			// THEN
			Mockito.verify(actionHandler).processReturningText(
					Mockito.isA(String.class), Mockito.eq(get));
		} finally {
			server.stopSilent();
		}
